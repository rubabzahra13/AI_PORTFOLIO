...
export default async function ProjectPage({ params }: Props) {
  ...
  const desktopScreenshots = project.images.filter((img) => !img.src.includes('mobile-'));
  const mobileQueueScreenshots = project.images.filter((img) => img.src.includes('mobile-'));

  const screenshotPairPortrait =
    desktopScreenshots.length === 2 &&
    desktopScreenshots.every((img) => (img.height ?? 0) > (img.width ?? 0));
        ...
        <section className="relative py-16 sm:py-20 overflow-hidden">
          ...
          <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 pt-10">
            ...
            {desktopScreenshots.length > 0 && (
              <div
                className={
                  screenshotPairPortrait
                    ? 'grid grid-cols-2 gap-3 sm:gap-8 items-start'
                    : 'space-y-10'
                }
              >
                {desktopScreenshots.map((img, i) => {
                  const isPortrait = (img.height ?? 0) > (img.width ?? 0);
                  return (
                    <figure
                      key={img.src + String(i)}
                      className={`group min-w-0 overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-white/[0.1] transition-all duration-500 ${
                        isPortrait && screenshotPairPortrait
                          ? 'w-full'
                          : isPortrait
                            ? 'mx-auto w-full max-w-[min(100%,260px)] sm:max-w-[min(100%,300px)]'
                            : ''
                      }`}
                    >
                      <div className="relative overflow-hidden">
                        <Image
                          ...
                          sizes={
                            isPortrait && screenshotPairPortrait
                              ? '(max-width: 640px) 45vw, min(360px, 50vw)'
                              : isPortrait
                      ...
                      {(img.caption ?? img.alt) && (
                        <figcaption
                          className={`px-2 py-2 sm:px-4 sm:py-3 text-zinc-400 border-t border-white/[0.04] ${
                            screenshotPairPortrait ? 'text-xs sm:text-sm leading-snug' : 'text-sm'
                          }`}
                        >
                          {img.caption ?? img.alt}
                        </figcaption>
            ...
            {mobileQueueScreenshots.length > 0 && (
              <div
                className={
                  desktopScreenshots.length > 0 ? 'mt-10 sm:mt-12' : ''
                }
              >
                <div
                  className="flex flex-row flex-nowrap gap-3 sm:gap-4 overflow-x-auto pb-3 pt-1 [-webkit-overflow-scrolling:touch] snap-x snap-mandatory -mx-4 px-4 sm:mx-0 sm:px-0 scroll-pl-4 sm:scroll-pl-0"
                  role="list"
                  aria-label="Mobile screenshots"
                >
                  {mobileQueueScreenshots.map((img) => {
                    const isPortrait = (img.height ?? 0) > (img.width ?? 0);
                    return (
                      <figure
                        key={img.src}
                        role="listitem"
                        className="snap-start shrink-0 w-[min(44vw,220px)] sm:w-[220px] min-w-[min(44vw,220px)] sm:min-w-[220px] overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-white/[0.1] transition-all duration-500 group"
                      >
                        <div className="relative overflow-hidden">
                          <Image
                            src={img.src}
                        ...
                        {(img.caption ?? img.alt) && (
                          <figcaption className="px-2 py-2 sm:px-3 sm:py-2.5 text-zinc-400 border-t border-white/[0.04] text-xs sm:text-sm leading-snug">
                            {img.caption ?? img.alt}
                          </figcaption>
                        )}
                      </figure>
