          ...
          {project.images.length > 0 && (
            <div>
                ...
                <div
                  className={
                    screenshotPairPortrait
                      ? 'grid grid-cols-2 gap-2 sm:gap-4 items-start mt-4'
                      : 'space-y-4 mt-4'
                  }
                >
                  {desktopScreenshots.map((img, i) => {
                    ...
                    return (
                      <figure
                        ...
                        className={`min-w-0 overflow-hidden rounded-xl border border-white/[0.06] ${
                          isPortrait && screenshotPairPortrait
                            ? 'w-full'
                            : isPortrait
                              ? 'mx-auto w-full max-w-[min(100%,220px)] sm:max-w-[min(100%,260px)]'
                              : ''
                        }`}
                      >
                        <Image
                          ...
                          sizes={
                            isPortrait && screenshotPairPortrait
                              ? '(max-width: 640px) 42vw, min(260px, 45vw)'
                              : isPortrait
                        ...
                        {(img.caption ?? img.alt) && (
                          <figcaption
                            className={`px-1.5 py-1.5 sm:px-3 sm:py-2 text-zinc-400 border-t border-white/[0.04] ${
                              screenshotPairPortrait
              ...
              {mobileQueueScreenshots.length > 0 && (
                ...
                >
                  <div
                    className="flex flex-row flex-nowrap gap-3 overflow-x-auto pb-2 pt-0.5 [-webkit-overflow-scrolling:touch] snap-x snap-mandatory scroll-pl-1 sm:scroll-pl-0"
                    role="list"
                    ...
                    {mobileQueueScreenshots.map((img) => {
                      ...
                      return (
                        <figure
                          key={img.src}
                          role="listitem"
                          className="snap-start shrink-0 w-[min(46vw,200px)] sm:w-[200px] min-w-[min(46vw,200px)] sm:min-w-[200px] overflow-hidden rounded-xl border border-white/[0.06]"
                        >
                          <Image
                            src={img.src}
                            alt={img.alt}
                          ...
                          />
                          {(img.caption ?? img.alt) && (
                            <figcaption className="px-1.5 py-1.5 sm:px-2 sm:py-2 text-zinc-400 border-t border-white/[0.04] text-[10px] sm:text-xs leading-tight">
                              {img.caption ?? img.alt}
                            </figcaption>
                          )}
                        </figure>
